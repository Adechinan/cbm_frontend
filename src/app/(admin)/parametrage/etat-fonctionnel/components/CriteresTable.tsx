...
import { deleteCritere, deleteElementFonctionnel } from '@/services/batimentService'
...
export default function CriteresTable({ criteres: init }: Props) {
  ...
  const handleDeleteElement = async (sectionId: string, elementId: string) => {
    ...
    const updated = await deleteElementFonctionnel(sectionId, elementId)
                  ...
                  {sec.elements.length === 0 && (
                    <tr>
                      <td colSpan={6} className="text-center text-muted py-3 fst-italic small">
                        Aucun élément — <button className="btn btn-link btn-sm p-0" type="button" onClick={() => openAddElement(sec.id)}>Ajouter le premier</button>
                      </td>
                    </tr>
                  )}
                </tbody>
